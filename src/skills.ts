...
export async function useSkill(skillsDir: string, name: string): Promise<string> {
...
export function buildUseSkillTool(skillsDir: string): AgentTool {
  return {
    name: "use_skill",
    label: "Use skill",
    description: "读取技能目录中指定 skill 的 SKILL.md；只能读取技能目录，不执行任意文件操作。",
    parameters: Type.Object({ name: Type.String({ description: "技能名称" }) }),
    execute: async (_toolCallId, args) => ({ content: [{ type: "text", text: await useSkill(skillsDir, String((args as { name: string }).name)) }], details: {} }),
  };
}
